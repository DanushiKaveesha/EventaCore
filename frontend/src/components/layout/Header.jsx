      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <Link to="/" className="flex items-center space-x-2 group shrink-0">
            <img src={logoImg} alt="EventraCore Logo" className="h-10 w-10 sm:h-12 sm:w-12 object-contain rounded drop-shadow-sm group-hover:scale-105 transition-transform" />
          ...
          <div className="flex items-center space-x-4">
            <div className="hidden sm:flex items-center space-x-1">
              {user && (
                <>
                  ...
                  <Link to="/settings" className="relative p-2 text-gray-500 hover:text-[#7C3AED] transition-colors rounded-full hover:bg-[#7C3AED]/10 flex items-center justify-center" title="Settings">
            ...
            <div className="hidden lg:flex items-center space-x-3 pl-4 border-l border-gray-100">
              {user ? (
                <>
                  <Link 
                    to={location.pathname === '/dashboard' ? '/profile' : '/dashboard'} 
                    className="flex items-center space-x-2 pl-3 pr-1 py-1 rounded-2xl hover:bg-slate-50 border border-transparent hover:border-gray-100 transition-all cursor-pointer group"
                    title={location.pathname === '/dashboard' ? 'View Profile Details' : 'Dashboard'}
                  >
                    <div className="text-right flex items-center h-full sm:flex hidden pr-2">
                      <p className="text-sm font-black text-gray-900 leading-tight truncate max-w-[120px] group-hover:text-indigo-600 transition-colors">{user.firstName || user.name || 'User'}</p>
                    </div>
                    <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-blue-100 to-purple-50 border border-blue-200 flex items-center justify-center shadow-inner overflow-hidden">
