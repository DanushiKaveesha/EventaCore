        ...
        <div className="bg-white rounded-[40px] border border-gray-100 shadow-2xl shadow-slate-200/50 overflow-hidden">
          ...
          <div className="p-2">
            {loading ? (
              ...
              <div className="overflow-x-auto rounded-3xl border border-gray-100/80">
                <table className="w-full text-left">
                  <thead className="bg-gray-50/50 border-b border-gray-100/80">
                    <tr>
                      <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">User Info</th>
                      ...
                      <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Date Joined</th>
                      <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Status</th>
                      <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right pr-14">Actions</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={user._id} className="hover:bg-slate-50/40 transition-all group">
                        <td className="px-10 py-5">
                          <div className="flex items-center gap-3.5">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${getRowAvatarStyle(user.name)}`}>
                              {user.name?.charAt(0) || 'U'}
                            </div>
                            <div>
                              <p className="text-sm font-bold text-[#0f172a] group-hover:text-blue-600 transition-colors duration-200 whitespace-nowrap">{user.name}</p>
                              <span className="text-[11px] font-semibold text-gray-500 whitespace-nowrap">@{user.username || 'user'}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-10 py-5">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2 text-gray-800">
                              ...
                              <span className="text-xs font-semibold whitespace-nowrap">{user.email}</span>
                            </div>
                            <div className="flex items-center gap-2 text-gray-400">
                               <PhoneIcon className="w-4 h-4 text-gray-400" />
                               <span className="text-xs font-medium whitespace-nowrap">{user.contactNumber || 'No contact'}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-10 py-5">
                          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-gray-600 bg-gray-100 border border-gray-200 whitespace-nowrap">
                            <ShieldCheckIcon className="w-4 h-4 text-gray-400 shrink-0" />
                        ...
                        <td className="px-10 py-5">
                          <div className="flex items-center gap-2 text-gray-500">
                            ...
                            <span className="text-xs font-medium whitespace-nowrap">{user.createdAt ? new Date(user.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Apr 3, 2026'}</span>
                          </div>
                        </td>
                        <td className="px-10 py-5">
                            <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border ${
                              user.status === 'active' || (user.isActive && !user.status) ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                              user.status === 'suspended' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-red-50 text-red-700 border-red-100'
                            }`}>
                              <div className={`w-1.5 h-1.5 rounded-full ${
                                user.status === 'active' || (user.isActive && !user.status) ? 'bg-emerald-500' : 
                                user.status === 'suspended' ? 'bg-amber-500' : 'bg-red-500'
                              }`} />
                              {user.status ? user.status.charAt(0).toUpperCase() + user.status.slice(1).toLowerCase() : (user.isActive ? 'Active' : 'Locked')}
                            </div>
                        </td>
                        <td className="px-10 py-5 text-right pr-14">
                           <div className="flex items-center justify-end gap-2 text-gray-500">
                            <button 
                              onClick={() => { setMessagingUserId(user._id); setAdminMessage(''); }} 
                              className="w-8 h-8 flex items-center justify-center bg-white hover:bg-gray-50 rounded-full border border-gray-200 text-gray-400 hover:text-gray-600 transition-colors"
                              title="Message User"
                            >
                            ...
                            <select 
                              ...
                              className="bg-white border border-gray-200 text-gray-700 text-xs font-semibold rounded-lg px-3 py-1.5 outline-none focus:ring-2 focus:ring-indigo-500/20 cursor-pointer min-w-[90px] hover:border-gray-300 transition-colors"
                            >
                              <option value="active" className="text-blue-600 font-bold">Active</option>
                              <option value="suspended" className="text-gray-900 font-bold">Suspend</option>
                              <option value="deactivated" className="text-red-600 font-bold">Deactivate</option>
                            </select>
                          </div>
                        </td>
                      </tr>
            ...
            <div className="p-10 text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-gray-400 text-sm font-bold leading-relaxed px-4">
                  Changing status to <span className="text-amber-600 uppercase font-black tracking-widest">{statusConfirm.newStatus}</span> will immediately revoke this user's platform access. Are you sure?
                </p>
              </div>
            ...
            <form onSubmit={handleSendMessage} className="p-7 space-y-6">
              ...
              <div className="flex justify-end gap-4">
                <button type="submit" disabled={isMessageSending} className="w-full py-4 text-sm font-black text-white bg-blue-600 rounded-2xl shadow-xl shadow-blue-500/20 hover:bg-blue-700 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-3 disabled:opacity-50">
                  ...
                  {sendEmail ? 'Send message and email' : 'Send Message'}
                </button>
              </div>
            </form>
          </div>
